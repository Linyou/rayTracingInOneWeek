...
@ti.func
def scatter(i, j, attenuation: ti.template()):

    kind = hit_holder.record[i, j].material_kind
    albedo = hit_holder.record[i, j].material_albedo
    extra = hit_holder.record[i, j].material_extra
    did_scatter = False
    if kind == 0: # lambertian
        ...
        reflected = reflect(ray_holder.rays[i, j].direction.normalized(), hit_holder.record[i, j].normal)

        ray_holder.rays[i, j].origin = hit_holder.record[i, j].p
        ray_holder.rays[i, j].direction = reflected + extra*random_in_unit_sphere()

        attenuation = albedo

        did_scatter = reflected.dot(hit_holder.record[i, j].normal) > 0
        ...
        unit_direction = ray_holder.rays[i, j].direction.normalized()
        ...
        ray_holder.rays[i, j].origin = hit_holder.record[i, j].p
        ray_holder.rays[i, j].direction = hit_holder.record[i, j].normal
        attenuation = albedo
        did_scatter = True
...
class HitRecord:
    def __init__(self, res):
        self.record = ti.Struct.field({
            ...
            "material_albedo": ti.types.vector(3, ti.f32),
...
class Shpere:
    ...
    def hit(self, index, i, j, t_min, t_max):
        ...
        if hitting == True:
            ...
            hit_holder.record[i, j].material_kind = self.content[index].material_kind
            hit_holder.record[i, j].material_albedo = self.content[index].material_albedo
            hit_holder.record[i, j].material_extra = self.content[index].material_extra
...
while window.running:
    ...
    window.GUI.begin("Configuration", 0.05, 0.05, 0.3, 0.4)
    new_vfov = window.GUI.slider_float("vfov", vfov, 0.0, 100.0)
    if abs(new_vfov - vfov) > 0:
        vfov = new_vfov
        changing_factor = True

    new_max_depth = int(window.GUI.slider_float("max_depth", max_depth[None], 0, 100))
    if abs(new_max_depth - max_depth[None]) > 0:
        max_depth[None] = new_max_depth
        changing_factor = True

    new_oriented_movement = window.GUI.slider_float("oriented_movement", oriented_movement, -10.0, 10.0)
    if abs(new_oriented_movement - oriented_movement) > movement_speed:
        oriented_movement = new_oriented_movement
        changing_factor = True

    new_dist_to_focus = window.GUI.slider_float("dist_to_focus", dist_to_focus, 0.0, 30.0)
    if abs(new_dist_to_focus - dist_to_focus) > movement_speed:
        dist_to_focus = new_dist_to_focus
        changing_factor = True

    window.GUI.text("local_movement:")

    new_local_movement[0] = window.GUI.slider_float("localx", local_movement[0], -10.0, 10.0)
    if abs(new_local_movement[0] - local_movement[0]) > movement_speed:
        local_movement[0] = new_local_movement[0] 
        changing_factor = True

    new_local_movement[1] = window.GUI.slider_float("localy", local_movement[1], -10.0, 10.0)
    if abs(new_local_movement[1] - local_movement[1]) > movement_speed:
        local_movement[1] = new_local_movement[1]
        changing_factor = True

    window.GUI.text("look_at_change:")
    look_at_change
    new_look_at_change[0] = window.GUI.slider_float("lookAtx", look_at_change[0], -10.0, 10.0)
    if abs(new_look_at_change[0] - look_at_change[0]) > movement_speed:
        look_at_change[0] = new_look_at_change[0] 
        changing_factor = True

    new_look_at_change[1] = window.GUI.slider_float("lookAty", look_at_change[1], -10.0, 10.0)
    if abs(new_look_at_change[1] - look_at_change[1]) > movement_speed:
        look_at_change[1] = new_look_at_change[1]
        changing_factor = True

    new_look_at_change[2] = window.GUI.slider_float("lookAtz", look_at_change[2], -10.0, 10.0)
