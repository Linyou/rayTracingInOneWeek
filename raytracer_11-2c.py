...
def random_in_unit_sphere():
    ...
    p, length_squared = get_random_vector_length()
...
# Camera
class Camera:
    def __init__(self, lookfrom, lookat, vup, vfov, aspect_ratio, image_width):
        ...
        origin = ti.Vector([lookfrom[0], lookfrom[1], lookfrom[2]])
        horizontal = ti.Vector([u[0] * viewport_width, u[1] * viewport_width, u[2] * viewport_width])
        vertical = ti.Vector([v[0] * viewport_height, v[1] * viewport_height, v[2] * viewport_height])

        self.origin[None] = origin
        self.horizontal[None] = horizontal
        self.vertical[None] = vertical
        self.lower_left_corner[None] = origin - horizontal/2 - vertical/2 - w
        self.res = (image_width, image_height)
        self.samples_per_pixel = 10

    def update_camera(self, local_movement, oriented_movement, vfov):
    ...
    def set_ray(self, i, j, s, t):
        ray_holder.rays[i, j].origin = self.origin[None]
        ray_holder.rays[i, j].direction = self.lower_left_corner[None] + s * self.horizontal[None] + t * self.vertical[None] - self.origin[None]
